...
class ResNet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if self.image_variant is not None:
            ...
            '''
            #self.resnet = efficient_face()
            self.resnet.fc = nn.Linear(2048, 12666)
            self.resnet = torch.nn.DataParallel(self.resnet).cuda()
            checkpoint = torch.load("/home/swli2/slowfast/resnet50_pretrained_on_msceleb.pth.tar")
            ...
            #self.resnet.module.fc = nn.Linear(2048, 2).cuda()
            ...
            if self.image_variant == "video":
                ...
                #self.image_mlp = nn.Sequential(nn.Linear(1536, 768), nn.ReLU(), nn.Linear(768, self.num_classes))
                ...
                self.image_mlp = nn.Sequential(nn.Linear(self.feature_size, 2408), nn.ReLU(), nn.Linear(2408, self.num_classes))
            ...
            elif self.image_variant == "baseline+region":
        ...
        if self.use_time_embed:
            if "temporal_attention" in self.st_config and "region_attention" in self.st_config:
                self.time_embed = nn.Parameter(torch.zeros(1, 8, self.feature_size))
            else:
    ...
    def forward(self, x, bboxes): 
        ...
        if self.image_variant == "baseline":
            print(x[0].shape)
            out = x[0].reshape((batch_size, -1))
            print("FINAL", out.shape)
            out = self.image_mlp(out)
        ...
        if self.image_variant == "video":
            #(batch_size, num_frames, num_feature_maps, fmap_dim, fmap_dim)
            print(x[0].shape)
            ...
            print("Feature size", video_level_features.shape)
            ...
            print("FINAL SHAPE", video_level_features.shape)
            out = self.image_mlp(video_level_features)
            out = self.out_batchnorm(out)
            out = self.act(out)
        ...
        if self.image_variant == "region":
            ...
            print(feature_maps.shape)
            feature_maps = feature_maps.reshape((batch_size, -1))
            print("FINAL SHAPE", feature_maps.shape)
            out = self.image_mlp(feature_maps)
            out = self.out_batchnorm(out)
            out = self.act(out)
            return out
