...
class Block(nn.Module):
    ...
    def forward(self, x, B, T, W):
        ...
        print("num_spatial tokens: ", num_spatial_tokens)
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        B = x.shape[0]
        batch_size, num_frames, num_patches, patch_size = x.shape
        T = num_frames
        x = x.reshape((batch_size*num_frames, num_patches, patch_size))
        ...
        if x.size(1) != self.pos_embed.size(1):
            ...
            print("pos_embed.size(1): ", self.pos_embed.size(1))
            1/0
...
class TimeSformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, num_classes=400, num_frames=8, attention_type='divided_space_time', depth=1, embed_dim=768, pretrained_model='', **kwargs):
        super(TimeSformer, self).__init__()
        self.pretrained=True
        self.model = VisionTransformer(img_size=img_size, num_classes=num_classes, patch_size=patch_size, embed_dim=embed_dim, depth=depth, num_heads=12, mlp_ratio=4, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1, num_frames=num_frames, attention_type=attention_type, **kwargs)
