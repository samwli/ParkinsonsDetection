...
class ResNet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.image_variant = "region_baseline"
        ...
        self.proj = nn.Linear(25088, 9216)
        if self.image_variant is not None:
            ...
            if self.image_variant == "video":
                ...
                self.image_mlp = nn.Linear(1032192, self.num_classes) 
            elif self.image_variant == "video+region_baseline":
    ...
    def forward(self, x, bboxes): 
        ...
        if self.image_variant == "video+region_baseline":
            #(batch_size, num_frames, num_feature_maps, fmap_dim, fmap_dim)
            print(x[0].shape)
            x[0] = x[0].permute(0, 2, 1, 3, 4)
            x_shape = x[0].shape
            ...
            feature_maps = feature_maps.reshape((batch_size, -1))
            video_level_features = torch.cat((video_level_features, feature_maps), dim=1)
