...
class RegionClassificationHead(nn.Module):
    ...
    def forward(self, inputs):
        ...
        '''if not self.training:
            ...
            if x_proj.ndim == 4 and x_proj.shape[1:3] > torch.Size([1, 1]):
                x_proj = x_proj.mean([1, 2])
                '''
        
        x_proj = x_proj.view(x_proj.shape[0], -1)
        #x_proj = self.act(x_proj)
        print("HEAD OUTPUT_SHAPE", x_proj.shape)
        print("HEAD OUTPUT", x_proj)
...
class MLPHead(nn.Module):
    def __init__(
        ...
        mlp_layers = [nn.Linear(dim_in, 9216, bias=b)]
        ...
        for i in range(1, num_layers):
            ...
            if i == num_layers - 1:
                ...
                d = mlp_dim
            mlp_layers.append(nn.Linear(x, d, bias=b))
            mlp_layers[-1].xavier_init = xavier_init
...
class ResNetBasicHead(nn.Module):
    ...
    def forward(self, inputs):
        ...
        print("permuted and normalized x shape: ", x.shape)
        ...
        print("x shape before projection: ", x.shape)
        x_proj = self.projection(x)
        ...
        x_proj = x_proj.view(x_proj.shape[0], -1)
        print("x projection view shape: ", x.shape)
