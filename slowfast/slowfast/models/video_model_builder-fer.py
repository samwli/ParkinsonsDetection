...
# frozen feature backbone + linear layer - 75% top1 acc --> 
# frozen feature backbone + roialign + maxpooling + linear layer - 45.83% top1 acc ---> (18432, 2)
# feature backbone + lienar layer - 66% top1 acc
# feature backbone + roialign + maxpooling + linear layer - 41% top1 acc
...
class ResNet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.st_func = {
            "region_pool_temporal_attention": self.region_pool_temporal_attention,
            "temporal_pool_region_attention": self.temporal_pool_region_attention,
            "region_attention_temporal_pool": self.region_attention_temporal_pool,
            "temporal_attention_region_pool": self.temporal_attention_region_pool,
            "temporal_attention_region_attention": self.temporal_attention_region_attention,
            "temporal_pool_region_pool": self.temporal_pool_region_pool
        }
        ...
        self.use_mlp = False
        self.use_bn = True
        self.st_config = ["temporal_pool", "region_attention"]
        ...
        if self.image_variant is not None:
            ...
            if self.image_variant == "video":
                # self.image_mlp = nn.Linear(262144, self.num_classes) 
                self.image_mlp = nn.Linear(100352, self.num_classes) 
            elif self.image_variant == "region":
                # self.image_mlp = nn.Linear(129024, self.num_classes)
                self.image_mlp = nn.Linear(258048, self.num_classes)
                
            else:
                # self.image_mlp = nn.Linear(262144 + 129024, self.num_classes)
                self.image_mlp = nn.Linear(100352 + 258048, self.num_classes) 
        ...
        if self.use_vit:
            self.vit_proj = nn.Linear(18432, self.vit_embed_dim)
        ...
        if self.st_config is not None:
            self.temporal_attention = Attention(9216, num_heads=3) if "temporal_attention" in self.st_config else None
            self.region_attention = Attention(9216, num_heads=3) if "region_attention" in self.st_config else None
            self.cls_token = nn.Parameter(torch.zeros(1, 1, 9216))
            # trunc_normal_(self.cls_token, std=.02)

        if self.use_pos_embed:
            self.pos_embed = nn.Parameter(torch.zeros(1, 14+1, 9216))
            # trunc_normal_(self.pos_embed, std=.02)
        
        if self.use_time_embed:
            if "temporal_attention" in self.st_config and "region_attention" in self.st_config:
                self.time_embed = nn.Parameter(torch.zeros(1, 8, 9216))
            else:
                self.time_embed = nn.Parameter(torch.zeros(1, 8+1, 9216))
    ...
    def temporal_pool_region_attention(self, x):
        batch_size, num_frames, num_regions, region_fv = x.shape[0], x.shape[1], x.shape[2], x.shape[3]
    ...
    def temporal_attention_region_pool(self, x):
        batch_size, num_frames, num_regions, region_fv = x.shape[0], x.shape[1], x.shape[2], x.shape[3]
    ...
    def region_attention_temporal_pool(self, x):
        batch_size, num_frames, num_regions, region_fv = x.shape[0], x.shape[1], x.shape[2], x.shape[3]
        ...
        final_cls = final_cls.reshape((batch_size, num_frames, -1))
        final_cls = torch.mean(final_cls, 1, True)
        final_cls = torch.squeeze(final_cls)
        return final_cls
    ...
    def temporal_pool_region_pool(self, x):
        batch_size, num_frames, num_regions, region_fv = x.shape[0], x.shape[1], x.shape[2], x.shape[3]
    ...
    def forward(self, x, bboxes): 
        ...
        if self.use_imagenet_resnet:
            ...
            # output is of shape [32, 2048, 7, 7]
        ...
        if self.image_variant == "video+region":
            ...
            feature_maps = feature_maps.reshape((batch_size, -1))
            video_region_features = torch.cat((video_level_features[0], feature_maps), dim=1)
            out = self.image_mlp(video_region_features)
