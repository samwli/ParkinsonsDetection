...
class GradCAM:
    ...
    def _calculate_localization_map(self, inputs, labels=None, bboxes=None):
        """
        ...
        for i, inp in enumerate(inputs):
            ...
            print("Gradients size for vis", gradients.size())
            print("Activations size", activations.size())
            B, C, Tg, x, y = gradients.size()
            ...
            print("localization map shape", localization_map.size())
            ...
            print("normalized map shape", localization_map.size())

            localization_maps.append(localization_map)

        return localization_maps, preds

    def __call__(self, inputs, labels=None, alpha=0.5, bboxes=None):
        """
        ...
        localization_maps, preds = self._calculate_localization_map(
            inputs, labels=labels, bboxes=bboxes
        )
        ...
        for i, localization_map in enumerate(localization_maps):
            ...
            print("curr_inp shape", curr_inp.shape)
