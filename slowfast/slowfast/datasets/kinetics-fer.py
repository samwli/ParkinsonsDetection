...
class Kinetics(torch.utils.data.Dataset):
    ...
    def __init__(self, cfg, mode, num_retries=100):
        ...
        with open("./data_images/bbox.pickle", 'rb') as f:
            self.bboxes_dict = pickle.load(f)
        '''
    ...
    def _construct_loader(self):
        ...
        path_to_bbox_file = os.path.join(
            self.cfg.DATA.PATH_TO_DATA_DIR, "{}_bboxes.csv".format(self.mode)
        )
        assert pathmgr.exists(path_to_bbox_file), "{} dir not found".format(
            path_to_bbox_file
        )
        ...
        with pathmgr.open(path_to_bbox_file, "r") as f:
            ...
            for clip_idx, path_label in enumerate(rows):
                ...
                self._path_to_bboxes.append(
                    os.path.join(self.cfg.DATA.PATH_PREFIX, path)
                )

        assert (
            len(self._path_to_bboxes) > 0
        ), "Failed to load Bbox split {} from {}".format(
            self._split_idx, path_to_file
        )
        logger.info(
            "Constructing bbox dataloader (size: {} skip_rows {}) from {} ".format(
                len(self._path_to_bboxes), self.skip_rows, path_to_file
            )
        )
    ...
    def __getitem__(self, index):
        ...
        ################## Image Version ##################
        frame = cv2.imread(self._path_to_videos[index], cv2.IMREAD_COLOR)
        frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        frame = frame / 255.0
        label = self._labels[index]
        bboxes = np.load(self._path_to_bboxes[index])
        bboxes = torch.Tensor(bboxes)
        bboxes = torch.unsqueeze(bboxes, 0).float()

        frame = torch.unsqueeze(torch.from_numpy(frame), 0).float()
        frame = frame.permute((3, 0, 1, 2))

        return [frame], label, index, torch.empty((2, 3)), {}, bboxes
        ###################################################
        ...
        for i_try in range(self._num_retries):
            ...
            # print("true frames size: ", frames[0].shape)


            # print("frames shape: ", frames[0].shape)
            # input_tensor = frames[0].unsqueeze(0)
            # if not os.path.isdir("debug_outputs"):
            #     os.makedirs("debug_outputs")

            # os.makedirs("debug_outputs")
            # input_tensor = input_tensor.clone().detach().permute(0, 1, 4, 2, 3)
            # print("input tensor shape: ", input_tensor.shape)
            # vid_cnt = 0
            # for video in range(input_tensor.shape[0]):
            #     vid_writer = cv2.VideoWriter(f"./debug_outputs/atdataset_vid{vid_cnt}.avi",cv2.VideoWriter_fourcc('M','J','P','G'), 20, (256,256))
            ...
            #         cv2.imwrite(f"./debug_outputs/atdataset_vid{vid_cnt}_frame{curr_frame}.png", frame)
            #         # vid_writer.write(frame)
            #         curr_frame += 1
            #     vid_writer.release()
            #     vid_cnt += 1
            ...
            if index not in self.kpts:
                ...
                video_idx = index_str[34:-10] # ANDY, CHANGE THIS <--- 
